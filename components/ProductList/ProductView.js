    ...
    if(a !== b){
        ...
        scale-100 md:scale-90 p-5
         bg-zinc-700">
            ...
            {  user ?  <form className='flex flex-col' onSubmit={sendComent}>
                 {user ? <p> Write coment belove <strong>{user.email}</strong></p> : <strong>you need to bee loged in to comment</strong>}
             <input 
             className='w-auto h-[100px] text-black'
             type="text"
             ref={commentRef}
             placeholder={`what do you think about product ${user ? user.email : "guest"}`}
             >

             </input>
             <button className='button w-32 tracking-widest ' type="submit"> Submit</button>
                 </form> : <p className='text-xl font-bold flex'>Login to comment  
                 <p className='text-green-200 pl-1'> <Link href="/Login" passHref> (here) </Link></p>  </p>}
            </div>
